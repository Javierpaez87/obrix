...
/** === Tokens de estilo alineados con BudgetManagement === */
...
const iconPill = 'p-2 rounded-full bg-zinc-800 border border-white/10';
const field =
    ...
    <div className="space-y-6 text-white" style={{ ['--neon' as any]: NEON }}>
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center px-3 sm:px-4 py-2 rounded-lg text-black bg-[--neon]
                     hover:opacity-90 transition ring-1 ring-[--neon]/30 whitespace-nowrap"
        >
      ...
      {/* Contenedor principal (bordes sutiles como en referencia) */}
      <div className={`${cardBase}`}>
        {/* Tabs */}
        <div className="flex gap-1 bg-zinc-950/60 p-1 rounded-lg mb-5 border border-white/10">
          <button
          ...
          >
            <span className="hidden sm:inline-flex items-center gap-2">
              {/* usando outline Wrench si prefieres; mantengo el look */}
              <svg className="h-4 w-4 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                <path d="M14 7l-7 7M14 7l3 3M4 20l4-1 9-9" />
              </svg>
              Mano de obra ({laborContacts.length})
            </span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {list.map(contact => (
            <div key={contact.id} className={`${cardBase} flex flex-col h-full`}>
              {/* Header contacto */}
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <span className={`${chipBase} text-white/85 border-white/15 bg-zinc-800 mt-1`}>
                    <TagIcon className="h-3.5 w-3.5 mr-1 text-[--neon]" style={{ ['--neon' as any]: NEON }} />
                    {getSubcategoryLabel(contact.subcategory, contact.category)}
                  </span>
                </div>

                {contact.rating && contact.rating > 0 && (
                  <div className={iconPill} style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
              ...
              {contact.notes && (
                <div className="mb-4 p-2 bg-zinc-900/60 border border-white/10 rounded text-xs sm:text-sm text-white/80">
                  {contact.notes}
                </div>
              )}
              ...
              <div className="mt-auto">
                <div className="grid grid-cols-3 md:grid-cols-4 gap-2">
                  ...
                  {/* Presupuesto / Proyecto tipografía chica para que entre */}
                  {selectedCategory !== 'clients' ? (
                    <button
                      className="flex items-center justify-center gap-2 px-3 py-2 text-black bg-[--neon] rounded-md
                                 hover:opacity-90 transition ring-1 ring-[--neon]/30"
                      style={{ ['--neon' as any]: NEON }}
                    >
                  ...
                  {/* Eliminar -> verde con letras negras */}
                  <button
                    onClick={() => handleDeleteContact(contact.id)}
                    className="flex items-center justify-center gap-2 px-3 py-2 bg-[--neon] text-black rounded-md hover:opacity-90 transition ring-1 ring-[--neon]/30"
                    style={{ ['--neon' as any]: NEON }}
                  >
          ...
          <div className="bg-zinc-950 border border-white/10 rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
                    ...
                    {[1, 2, 3, 4, 5].map(star => (
                      ...
                      >
                        {star <= (newContact.rating || 0) ? (
                          <StarIconSolid className="h-6 w-6 text-[--neon]" style={{ ['--neon' as any]: NEON }} />
                        ) : (
            ...
            <div className="flex flex-col sm:flex-row justify-end gap-2 px-4 sm:px-6 py-4 border-t border-white/10">
              <button
                ...
                className="px-4 py-2 rounded-md text-white/90 border border-white/10 hover:bg-white/5 transition"
              >
                Cancelar
              </button>
              <button
